
namespace TeamWork.Common.ConstantNumbers
{
    public class Number
    {
        public static readonly int Number_1_Negative = -1;
        public static readonly int Number_0 = 0;
        public static readonly int Number_1 = 1;
        public static readonly int Number_2 = 2;
        public static readonly int Number_3 = 3;
        public static readonly int Number_100 = 100;
        public static readonly int Number_200 = 200;
        public static readonly int Number_201 = 201;
        public static readonly int Number_204 = 204;
        public static readonly int Number_400 = 400;
        public static readonly int Number_404 = 404;
        public static readonly int Number_409 = 409;
        public static readonly int Number_1000000 = 1000000;
        public static readonly int Position_1 = 1;
        public static readonly int Position_0 = 0;

    }
}
